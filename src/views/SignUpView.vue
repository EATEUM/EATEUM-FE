...
import { alertSuccess, alert, alertWarning } from '@/composables/useAlert'
...
const checkEmailDuplicate = async () => {
  ...
  try {
    ...
    if (error.response && error.response.status === 409) {
      alertWarning('이미 사용 중인 이메일입니다.', { title: '중복 확인' })
      isEmailAvailable.value = false
    } else {
...
const handleSubmit = async () => {
  if (!isEmailAvailable.value) {
    alertWarning('이메일 중복 확인을 해주세요.', { title: '확인 필요' })
    return
  }

  if (password.value !== passwordConfirm.value) {
    alertWarning('비밀번호가 서로 일치하지 않습니다.', { title: '입력 오류' })
    return
  }
